function [ FBsPCA_data, Timing, res_em, res_op]=hMRA_EM_nonuniform( Nclus, Ncopy, Ndir, SNR, numcoef, shift_range, file_name)
...
for I=1:Nclus
    for i=1:Ncopy(I)
        ...
        data(:,:,i+sum(Ncopy(1:I-1)))=rotdata(shift_range+1+shift(1):shift_range+sidel+shift(1),shift_range+1+shift(2):shift_range+sidel+shift(2));
    end
end
clear D
...
option.maxinner=25;
option.maxiter=1000;
[z, zcost, inform] = trustregions(problem,[],option);
...
L=length(Freqs);
lf=sum(Freqs==0);
x_init = [randn(lf,Nclus);randn(L-lf,Nclus)+1i*randn(L-lf,Nclus)];

x = x_init;
data = Coeff;
data(lf+1:L,:) = data(lf+1:L,:)*sqrt(2);
for iter=1:niter
    [x_new, W] = EM_iteration(x, data, sigma, Nclus, Ndir);
    dis = im_dist(x_new,x);
    if dis<Nclus*tol
        break;
    end
...
x(lf+1:L,:) = x(lf+1:L,:)/sqrt(2);
info = W;
Timing.t_em = cputime-TT;
res_em.iter = iter;
clear data W
...
res_em.image0=image0;
res_em.imager=imager;
res_em.ZA=ZA;
res_em.z=z;
